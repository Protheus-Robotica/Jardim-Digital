...
void loop() {
  ...
  /*Aceleração*/
  accZ = (int16_t)((i2c_data[0] << 8) | i2c_data[1]);  // ([ MSB ] [ LSB ])
  accY = (int16_t)((i2c_data[2] << 8) | i2c_data[3]);  // ([ MSB ] [ LSB ])
  accX = (int16_t)((i2c_data[4] << 8) | i2c_data[5]);  // ([ MSB ] [ LSB ])

  /*Giroscópio*/
  gyroZ = (int16_t)((i2c_data[8] << 8) | i2c_data[9]);    // ([ MSB ] [ LSB ])
  gyroY = (int16_t)((i2c_data[10] << 8) | i2c_data[11]);  // ([ MSB ] [ LSB ])
  gyroX = (int16_t)((i2c_data[12] << 8) | i2c_data[13]);  // ([ MSB ] [ LSB ])
